...
export class Guard {
  ...
  // eslint-disable-next-line complexity
  public static isBetween(value: number | string | unknown[], min: number, max: number): boolean {
    if (min > max) throw new GuardException(`Max ${max} should be greater than min ${min}.`);
